...
func (s *ScenarioRunner) setupStack(config *StackConfig) (*devstack.DevStack, *system.CleanupManager) {
	...
	if config.ComputeConfig.TotalResourceLimits.IsZero() {
		...
		cfg, err := node.NewComputeConfigWithDefaults(s.Config)
		s.Require().NoError(err)
	...
	stack := testutils.Setup(s.Ctx, s.T(), s.Repo, s.Config,
		append(config.DevStackOptions.Options(),
			...
			testutils.WithNoopExecutor(config.ExecutorConfig, s.Config),
		)...,
	)
...
func (s *ScenarioRunner) RunScenario(scenario Scenario) string {
	...
	apiClient, err := client.NewAPIClient(client.NoTLS, s.Config, apiServer.Address, apiServer.Port)
	s.Require().NoError(err)
	...
	if scenario.ResultsChecker != nil {
		...
		s.Config.User().Set(types.NodeIPFSSwarmAddresses, swarmAddresses)
		s.Config.User().Set(types.NodeIPFSPrivateInternal, true)
		...
		ipfsDownloader := ipfs.NewIPFSDownloader(cm, s.Config)
		s.Require().NoError(err)
