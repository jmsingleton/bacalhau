...
func NewLibp2pTransport(ctx context.Context,
	...
	gossipSub, err := newLibp2pPubSub(ctx, libp2pHost, cfg)
	if err != nil {
		return nil, err
	}
...
func newLibp2pPubSub(ctx context.Context, host host.Host, c pkgconfig.Context) (*libp2p_pubsub.PubSub, error) {
	tracer, err := libp2p_pubsub.NewJSONTracer(pkgconfig.GetLibp2pTracerPath(c))
	if err != nil {
		return nil, err
	}
