...
func newDockerRunCmd(cfg config.Context) *cobra.Command { //nolint:funlen
	...
	dockerRunCmd := &cobra.Command{
		...
		PreRunE:  hook.Chain(hook.RemoteCmdPreRunHooks, configflags.PreRun(cfg, dockerRunFlags)),
...
func dockerRun(cmd *cobra.Command, cfg config.Context, cmdArgs []string, opts *DockerRunOptions) error {
	ctx := cmd.Context()
	...
	executingJob, err := util.ExecuteJob(ctx, j, cfg)
	if err != nil {
		return err
	}
	...
	return printer.PrintJobExecutionLegacy(ctx, executingJob, cmd, cfg, opts.DownloadSettings, opts.RunTimeSettings, api)
}
